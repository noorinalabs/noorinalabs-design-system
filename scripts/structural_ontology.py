#!/usr/bin/env python3
"""Generate / staleness-check design-system's structural ontology index (#130).

This is the noorinalabs-design-system (consumer) side of the C×T2 distributed
...
``docs/devops/ontology-structural.md`` in noorinalabs-main.
...
* ``register-merge-driver`` — register the union merge-driver in git config (the
  ...
  local state, so ``make setup-hooks`` or a manual invocation runs this).
  NOTE: the driver is invoked in PYTHON MODULE form
  (``python3 -m ontology_gen.merge_driver``), NOT as a bare script —
  ``merge_driver.py`` uses a package-relative import (``from .model``),
...
REPO_NAME = "noorinalabs-design-system"
