#!/usr/bin/env bash
# Apply the noorinalabs-design-system default-branch protection ruleset.
#
...
#   * Required status checks: design-system has unconditional PR CI (ci.yml has
#     no paths filter), so the ruleset hard-requires its gate contexts (job
#     NAMES, not workflow names): `ci` and `validate-package`. Confirm them
#     against live check-runs at apply time
#     (`gh api repos/<repo>/commits/<default-sha>/check-runs`) — CI job names
#     can change. The docs.yml gates added in W14 (precommit-ci-sync,
#     markdownlint, spellcheck, linkcheck, config-lint, actionlint) are
#     paths-filtered and so are NOT hard-required here; add a context to the
#     required set only if its workflow is made unconditional.
...
#
# Usage:
#   ./apply-ruleset.sh                 # apply to noorinalabs/noorinalabs-design-system
...
REPO="${REPO:-noorinalabs/noorinalabs-design-system}"
